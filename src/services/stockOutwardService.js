 ...
 * ======================
 * V2: Supports sub_types for inverter + panel.
 */
 ...
 * @param {Object} data
 *   - from_district, dispatch_type, brand, dcr_type, created_by
 *   - dealer_id?, customer_name?, customer_district?, registered_customer_id?, to_district?
 *   - connector, notes
 *   - systems: { '2KW': 3, ... }
 *   - panel_breakdown: { '570': 2, '580': 10, ... }
 *   - items: [{ component, sub_type?, actual_quantity }]   (overrides for simple components)
 */
async function create(data) {
    const {
        from_district, dispatch_type, dealer_id, customer_name, customer_district,
        registered_customer_id, to_district, connector, brand, dcr_type,
    ...
    // Planned totals + inverter breakdown
    ...
    try {
        ...
        // 1. Insert outward header
        const [headerResult] = await conn.execute(
            `INSERT INTO stock_outward 
             (from_district, dispatch_type, dealer_id, customer_name, customer_district, 
              registered_customer_id, to_district, connector, brand, dcr_type, notes, created_by) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                ...
                connector || null, brand, dcr_type, notes || null, created_by,
            ]
        );
